...
class AuthService extends AbstractServices {
  ...
  public async login(req: Request, res: Response) {
    const { emailorPhone, password } = req.body as ILogin;

    return await this.models.db.transaction(async (trx) => {
      const auth_conn = this.models.authModel(req);
   
  
/* the functing is checking is it email or phone */
      const isemailOrphone= await auth_conn.validateEmailOrPhoneNumber(emailorPhone)
      if(isemailOrphone==='Email'){
        const userIsExist = await auth_conn.checkExistingEmail(emailorPhone);
        if (!userIsExist) {
          throw new CustomError("No user exist with this email", 400, "Error");
        }
      
      }  else if(isemailOrphone==='Phone'){
        const userIsExist = await auth_conn.checkExistingPhoneNumber(emailorPhone);
        if (!userIsExist) {
          throw new CustomError("No user exist with this PhoneNumber", 400, "Error");
        }
      }
      const user = await auth_conn.login({emailorPhone:emailorPhone,isEmailOrPhone: isemailOrphone==='Email'?'email':'phone'});
      /* user je pass dise seta same ache ki na */
  ...
  public async signUp(req: Request, res: Response) {
    const { email, password, birthday, name, gender, phoneNumber } =
    ...
    return await this.models.db.transaction(async (trx) => {
      ...
      const checkExistingPhoneNumber = await auth_conn.checkExistingPhoneNumber(phoneNumber);

      if (checkAlreadyExist?.userId) {
        throw new CustomError("Email already exists", 409, "Conflict");
      }
      if (checkExistingPhoneNumber?.userId) {
        throw new CustomError("PhoneNumber already exists", 409, "Conflict");
      }
